...
import { 
  GraduationCap, Lock, Mail, AlertCircle, 
  ArrowRight, ShieldCheck, CheckCircle2, Loader2,
  Sun, Moon, Building2, User, KeyRound, Eye, EyeOff,
  BookOpen, Landmark, Shield
} from 'lucide-react';
...
const AUTHORIZED_ACCOUNTS: PredefinedAccount[] = [
  {
    label: 'Student (Pending)',
    roleType: 'Graduating Student (Clearance In-Progress)',
  ...
  },
  {
    ...
    roleType: 'Graduating Student (Certified & Cleared)',
  ...
  {
    label: 'Library Officer',
    roleType: 'Department Clearance Officer (Library)',
    email: 'library@hu.edu.et',
    password: '123456',
    icon: BookOpen,
    badgeColor: 'text-blue-700 dark:text-blue-400',
    badgeBg: 'bg-blue-50 dark:bg-blue-950/60 border-blue-200/80 dark:border-blue-800/80'
  },
  {
    label: 'University Registrar',
    roleType: 'Central Registrar Directorate',
  ...
  },
  {
    ...
    roleType: 'System Administrator (Full Central Access)',
    email: 'superadmin@hu.edu.et',
    password: '123456',
    icon: Shield,
    badgeColor: 'text-purple-700 dark:text-purple-400',
    badgeBg: 'bg-purple-50 dark:bg-purple-950/60 border-purple-200/80 dark:border-purple-800/80'
  }
];
...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const handleSelectAccount = (account: PredefinedAccount) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Check if email belongs to predefined accounts or valid store accounts
      const matchedPredefined = AUTHORIZED_ACCOUNTS.find(
        acc => acc.email.toLowerCase() === cleanEmail.toLowerCase()
      );

      if (matchedPredefined) {
        if (password !== matchedPredefined.password) {
        ...
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-black/40 p-6 sm:p-8 space-y-6 transition-all">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80 space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono-code">
                Password: 123456
              </span>
            </div>

            <div className="grid grid-cols-1 gap-2">
              {AUTHORIZED_ACCOUNTS.map((acc, index) => {
                const Icon = acc.icon;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`p-1.5 rounded-lg border ${acc.badgeBg} ${acc.badgeColor} shrink-0`}>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] font-mono-code text-blue-600 dark:text-blue-400 truncate">
                          {acc.email}
                        </p>
                      </div>
                    </div>
                    <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 group-hover:text-blue-600 dark:group-hover:text-blue-400 shrink-0 ml-2">
                      Auto-fill
                    </span>
                  </button>
                );
